...
// querySelector letds you select by any CSS selector (any way you style the element)
...
function addToEnd(){
    ...
    // get the content of the text are, and add it as text to the new list item
    newListItem.textContent = document.querySelector("#pContent").value;

    // add the new list item to the UL (with the ID adder)
    const adder = document.querySelector("#adder");

    // check to se if there are a certain number of childer (more than 16)
    // and if there are, make the text white

    if(adder.children.length > 9) {
